...
function PgStore(pg, conString, options) {
  if (!(this instanceof PgStore)) {
    return new PgStore(pg, conString, options || {});
  }

  this._initPgQuery(pg, conString);
...
PgStore.prototype._initPgQuery = function (pg, conString) {
  ...
  this._pg = pg.connect.bind(pg.connect, conString);
};
